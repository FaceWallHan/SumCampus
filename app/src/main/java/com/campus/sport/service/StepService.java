...
import com.campus.sport.utils.step.StepCount;
import com.campus.sport.utils.step.StepValuePassListener;
import com.campus.sport.utils.step.UpdateUiCallBack;
import com.campus.sport.activity.MainActivity;
import com.campus.sport.sql.DbUtils;
import com.campus.sport.sql.bean.StepData;
...
public class StepService extends Service implements SensorEventListener {
    ...
    private void initTodayData() {
        ...
        if (list.size() == 0 || list.isEmpty()) {
            ...
            Log.i(TAG, "StepData=" + list.get(0).toString());
...
//                    Logger.d("重置步数" + StepDcretor.CURRENT_STEP);
    ...
    private void isNewDay() {
        String time = "00:00";
        if (time.equals(new SimpleDateFormat("HH:mm", Locale.CHINESE).format(new Date())) || !CURRENT_DATE.equals(getTodayDate())) {
            initTodayData();
        }
    }
     ...
     */
    private void isCall() {
        String time = AppClient.getParam(DataKeys.achieveTime, "20:00")+"";
        String plan = AppClient.getParam(DataKeys.planWalk, "7000")+"";
        boolean remind =(boolean) AppClient.getParam(DataKeys.remind, true);
        Log.i( "isCall: ","time=" + time + "\n" +  "new SimpleDateFormat(\"HH: mm\").format(new Date()))="  + new SimpleDateFormat("HH:mm",Locale.CHINESE).format(new Date()));
        if ((remind) && (time.equals(new SimpleDateFormat("HH:mm",Locale.CHINESE).format(new Date())))) {
            if (CURRENT_STEP < Integer.parseInt(plan)){
                remindNotify("距离目标还差" + (Integer.valueOf(plan) - CURRENT_STEP) + "步，加油！");
            }else if (CURRENT_STEP > Integer.parseInt(plan)){
                DecimalFormat format=new DecimalFormat("00.00");
                remindNotify("今日步数:"+plan+"步，超出目标" + format.format(CURRENT_STEP-Integer.parseInt(plan)) + "%，请继续保持！");
            }else {
                remindNotify("今日完成目标" + (Integer.valueOf(plan) - CURRENT_STEP) + "步，再接再励！");
            }
     ...
     * 提醒锻炼通知栏
     */
    private void remindNotify(String type) {
    ...
    public void onSensorChanged(SensorEvent event) {
        if (stepSensorType == Sensor.TYPE_STEP_COUNTER) {
            ...
            Log.d("onSensorChanged: ","tempStep" + tempStep);
    ...
    private void addBasePedometerListener() {
        ...
        if (isAvailable) {
            Log.i(TAG, "加速度传感器可以使用");
        } else {
            Log.i(TAG, "加速度传感器无法使用");
        }
    }
