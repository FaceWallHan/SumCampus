package com.campus.sport.utils.step;
...
public class StepArcView extends View {
     ...
     * 圆弧的宽度
     */
    private float borderWidth = dipToPx(15);
    /**
     * 画步数的数值的字体大小
     */
    ...
    protected void onDraw(Canvas canvas) {
        ...
        /**指定圆弧的外轮廓矩形区域*/
        Log.d( "onDraw: ","borderWidth"+borderWidth);
        RectF rectF = new RectF(borderWidth , 0+borderWidth,2 * centerX - borderWidth, 2 * centerX - borderWidth);
    ...
    private void drawArcYellow(Canvas canvas, RectF rectF) {
        ...
        /** 设置画笔的样式 Paint.Cap.Round ,Cap.SQUARE等分别为圆形、方形  (起始点和结束点的样式)*/
    ...
    private void drawArcRed(Canvas canvas, RectF rectF) {
        ...
        //步数再多也不能变成圆
        if (currentAngleLength>300){
            currentAngleLength=300;
        }
        if (currentAngleLength==0){
            currentAngleLength=0.001f;
        }
    ...
    private void drawTextStepString(Canvas canvas, float centerX) {
        String index= (String) AppClient.getParam(DataKeys.planWalk,"7000");
